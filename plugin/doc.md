...
There are no arbitrary restrictions/requirements on package name, component name and content
 ...
 name, use `com.github.baidutap.plugin.$PLUGIN_ID` if it only contains a single plugin to prevent
...
* MUST have action: `com.github.baidutap.plugin.ACTION_CONFIGURE`;
* MUST have category: `android.intent.category.DEFAULT`;
* MUST be able to receive data URI `plugin://com.github.baidutap/$PLUGIN_ID`;
* SHOULD parse string extra `com.github.baidutap.plugin.EXTRA_OPTIONS` (all options as a single
...
* On finish, it SHOULD return one of the following results:
  - `RESULT_OK = 0`: In this case it MUST return the data Intent with the new
    `com.github.baidutap.plugin.EXTRA_OPTIONS`;
...
This corresponds to `com.github.baidutap.plugin.ConfigurationActivity` in the plugin library.
...
* MUST have action: `com.github.baidutap.plugin.ACTION_HELP`;
* MUST have category: `android.intent.category.DEFAULT`;
* MUST be able to receive data URI `plugin://com.github.baidutap/$PLUGIN_ID`;
* CAN parse string extra `com.github.baidutap.plugin.EXTRA_OPTIONS` and display some more
  relevant information;
* SHOULD parse `@NightMode` int extra `com.github.baidutap.plugin.EXTRA_NIGHT_MODE` and act
  accordingly;
* SHOULD either:
  - Be invisible and return help message with CharSequence extra
    `com.github.baidutap.plugin.EXTRA_HELP_MESSAGE` in the data intent with `RESULT_OK`; (in this
...
This corresponds to `com.github.baidutap.plugin.HelpActivity` or
 `com.github.baidutap.plugin.HelpCallback` in the plugin library. Here's what a proper help
...
* MUST have an intent filter with action `com.github.baidutap.plugin.ACTION_NATIVE_PLUGIN`;
  (used for discovering plugins)
* MUST have meta-data `com.github.baidutap.plugin.id` with string value `$PLUGIN_ID` or a string resource;
* MUST have an intent filter with action `com.github.baidutap.plugin.ACTION_NATIVE_PLUGIN` and
  data `plugin://com.github.baidutap/$PLUGIN_ID`; (used for configuring plugin)
* CAN have meta-data `com.github.baidutap.plugin.default_config` with string value or a string resource, default is empty;
...
* MUST implement `openFile` that for files returned in `query`, `openFile` with `mode = "r"` returns
  ...
  `content://com.github.baidutap.plugin.kcptun/kcptun`.

This corresponds to `com.github.baidutap.plugin.NativePluginProvider` in the plugin library.
...
* MUST implement `call` that returns absolute path to the entry executable as
  `com.github.baidutap.plugin.EXTRA_ENTRY` when having `method = "shadowsocks:getExecutable"`;
  (`com.github.baidutap.plugin.EXTRA_OPTIONS` is provided in extras as well just in case you
  need them)
...
Additionally, if your plugin only needs to supply the path of your executable without doing any extra setup work,
 you can use an additional `meta-data` with name `com.github.baidutap.plugin.executable_path`
...
```
<meta-data android:name="com.github.baidutap.plugin.version"
           android:value="1.0.0"/>
```
...
* MUST define meta-data `com.github.baidutap.plugin.id.aliases` in your plugin content provider with `android:value="alias"`,
  or use `android:resources` to specify a string resource or string array resource for multiple aliases.
* MUST be able to be matched by `com.github.baidutap.plugin.ACTION_NATIVE_PLUGIN` when invoked on alias.
...
<manifest>
    ...
    <application>
        ...
        <provider>
            ...
            <meta-data android:name="com.github.baidutap.plugin.id"
                       android:value="$PLUGIN_ID"/>
            <meta-data android:name="com.github.baidutap.plugin.aliases"
